        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          <div className="space-y-4">
            <Link href={`/${lang}`} className="flex items-center gap-2.5 text-white font-bold text-lg">
              <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-white/15"><GraduationCap className="w-5 h-5" /></div>{APP_NAME}
            </Link>
            <p className="text-sm text-blue-200/70 leading-relaxed">{t(APP_DESCRIPTION.es, APP_DESCRIPTION.en)}</p>
            <p className="text-xs text-blue-200/50">© {new Date().getFullYear()} {APP_NAME}</p>
          </div>
