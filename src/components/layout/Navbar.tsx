...
export function Navbar({ lang, dict, session }: { lang: string; dict: Dict; session: SessionInfo | null }) {
  ...
  const linkClass = (path: string) =>
    `text-sm font-medium transition-colors px-3 py-1.5 rounded-lg ${pathname.includes(path) ? "bg-white/15 text-white" : "text-white/80 hover:text-white hover:bg-white/10"}`;

  return (
    <header className="sticky top-0 z-50 bg-gradient-to-r from-[#0a3d62] via-[#135d8a] to-[#0a3d62] shadow-lg" role="banner">
      <nav className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <div className="flex items-center gap-2">
          <Link href={otherLangPath} className="text-xs px-2.5 py-1.5 rounded-lg bg-white/10 text-white/80 hover:bg-white/20 hover:text-white transition-colors">{lang === "es" ? "EN" : "ES"}</Link>
          {session ? (
            <button onClick={handleLogout} disabled={loggingOut} className="inline-flex items-center gap-1.5 text-sm text-white/70 hover:text-white transition-colors disabled:opacity-50 ml-1">
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden ml-1 p-2 rounded-lg text-white/80 hover:bg-white/10" aria-label="Menú">
